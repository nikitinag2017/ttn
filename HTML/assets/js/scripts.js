...
(function($) {
	...
	$(document).ready( function() {
		...
		$( '.sidebar-toggler-mobile' ).on( 'click', function() {
			$( this ).closest('.site-content').toggleClass( "active-hh-sidebar" );
		} );
		...
		$( document ).on("click", function(event) {
			if ( $( '.site-content' ).hasClass( 'active-hh-sidebar' ) ) {
				...
				if ( ! $( '.handheld-sidebar-toggle' ).is( event.target ) && 0 === $( '.handheld-sidebar-toggle' ).has( event.target ).length && ! $( '#secondary' ).is( event.target ) && 0 === $( '#secondary' ).has( event.target ).length && ! $( '.sidebar-toggler-mobile' ).is( event.target ) && 0 === $( '.sidebar-toggler-mobile' ).has( event.target ).length) {
	...
	// 	        $('.product-count').fadeOut();
	// 	   } else {
	// 	        $('.product-count').fadeIn();
	// 	        $('.product-count').addClass('fixed');
	// 	   }
...
$(document).ready(function() {
	if ($('.product-count').length>0) {
	    var previousScroll = 0,
	        headerOrgOffset = $('.product-count').offset().top;
	    
	    $('.product-count-wrap').height($('.product-count').height());
	    ...
	    $(window).scroll(function() {
	        ...
	        if(currentScroll > headerOrgOffset) {
	            if (currentScroll > previousScroll) {
	             ...
	             $('.product-count').removeClass('fixed');   
	        }
	        previousScroll = currentScroll;
	    });
    }

    $('.product-count__del').on('click', function(e) {
        e.preventDefault();
        $('.schema-filter-button__state_hidden').removeClass('schema-filter-button__state_hidden').addClass('schema-filter-button__state');
    });


    if ($('.f-styler').length>0) {
    	$('.f-styler').styler()
    }

    $('.show-more').on('click', function(e) {
        e.preventDefault();
        $(this).parents('.show-more-wrap').find('.show-more-container').toggleClass('active');
    });
    $(document).click(function (e){
		var div = $(".show-more-wrap");
		if (!div.is(e.target)
		    && div.has(e.target).length === 0) { 
			$('.show-more-container').removeClass('active');
		}
	});

	$('.input-item__label-link').on('click', function(e) {
        e.preventDefault();
        $(this).next('.input-item__question').toggleClass('opened');
    });
    $(document).click(function (e){
		var div = $(".input-item__label--ask");
		if (!div.is(e.target)
		    && div.has(e.target).length === 0) { 
			$('.input-item__question').removeClass('opened');
		}
	});
...
function sliderTabsStart() {
	if ($('.slider-tabs').length>0) {
		var $tab_s = $('.slider-tabs');
		if($(window).width() < 600) {
			$tab_s.not('.slick-initialized').slick({
			  	infinite: false,
				slidesToShow: 2,
				arrows: false,
				dots: true,
			...
			$('.slider-tabs').on('click', '.slick-slide', function (e) {
			    // $('.slider-tabs').each(function(indx, element){
					var index = $(this).index();
				    // if ($(this).slick('slickCurrentSlide') !== index) {
				    ...
				    console.log(index);
				  	...
				  	$('.slider-tabs').slick('slickGoTo', index);

				// });
			});

		} else{
			if($tab_s.hasClass('slick-initialized')) {
				$tab_s.slick("unslick");
			}
		}
